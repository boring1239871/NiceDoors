...
import { validateCadModel, calculatePrice, svgToPng } from '../../utils';
...
export const DesignerView: React.FC<DesignerViewProps> = ({ initialTemplateId, onAddToOrder }) => {
  ...
  useEffect(() => {
    ...
    let defaultType: SashType = 'casement'; let defaultDir: SashDirection = 'left';
    ...
    setCadModel({ id: `CAD-${Date.now()}`, templateId: currentTemplate.id, width: currentTemplate.defaultSize.width, height: currentTemplate.defaultSize.height, panels: currentTemplate.defaultPanels, panelConfigs: initialConfigs, transomHeight: 0, hasThreshold: currentTemplate.type === ProductType.DOOR, thresholdHeight: 30, showOpeningIndicators: true, glassColor: 'blue', profileColor: defaultColor, glassType: 'double', enableMullions: true });
    setSelectedPanelIndex(null);
  }, [currentTemplate]);

  useEffect(() => { if (cadModel && currentTemplate) setValidation(validateCadModel(cadModel, currentTemplate)); }, [cadModel, currentTemplate]);

  const handleGlobalChange = (key: keyof CadModel, value: any) => { if (cadModel) setCadModel(prev => prev ? ({ ...prev, [key]: value }) : null); };
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-white rounded-tl-[24px] shadow-inner border-l border-t border-gray-200">
      <DesignerSidebar activeCategory={activeCategory} setActiveCategory={setActiveCategory} filteredTemplates={filteredTemplates} selectedTemplateId={selectedTemplateId} setSelectedTemplateId={setSelectedTemplateId} cadModel={cadModel} selectedPanelIndex={selectedPanelIndex} setSelectedPanelIndex={setSelectedPanelIndex} activePanelConfig={activePanelConfig} handlePanelConfigChange={handlePanelConfigChange} handleGlobalChange={handleGlobalChange} handlePanelCountChange={handlePanelCountChange} currentTemplate={currentTemplate} validation={validation} onAddToOrder={handleAddToOrder} />
      <main className="flex-1 flex flex-col bg-[#F3F6F9] relative overflow-hidden" id="cad-canvas-container">
        <div className="absolute top-6 left-6 z-10 flex gap-3"><div className="bg-white rounded-full shadow-sm border border-gray-200 p-1 flex"><button onClick={() => setViewMode('realistic')} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${viewMode === 'realistic' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>效果</button><button onClick={() => setViewMode('wireframe')} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${viewMode === 'wireframe' ? 'bg-gray-900 text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>线稿</button></div></div>
        <div className="flex-1 flex overflow-hidden p-4"><div className="w-full h-full bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden relative">{isJsonView ? <div className="w-full h-full p-8 overflow-auto"><pre className="text-xs bg-slate-800 text-green-400 p-4 rounded-xl">{JSON.stringify(cadModel, null, 2)}</pre></div> : <CadCanvas model={cadModel} template={currentTemplate} selectedPanelIndex={selectedPanelIndex} onPanelSelect={setSelectedPanelIndex} viewMode={viewMode} />}</div></div>
      </main>
    </div>
  );
};